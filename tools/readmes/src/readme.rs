...
use crate::model::{Exercise, Parsable, TrackDataResult, TrackMetadata, TrackStats};
...
impl ReadmeGenerator {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        handlebars.register_helper("progress_emoji", Box::new(progress_emoji_helper));
    ...
    pub fn generate_track_readmes(
        ...
        for (track_name, exercises) in exercises_by_track {
            ...
            let track_data =
                self.prepare_track_data(track_name, exercises, &track_slug, &metadata);
            ...
            track_stats.insert(track_name.clone(), TrackStats {
    ...
    fn prepare_track_data(
        ...
        for exercise in exercises {
            let solution_path = if exercise.local_path.starts_with(&format!("./{track_slug}/"))
            {
                exercise
                    .local_path
    ...
    fn prepare_main_data(
        ...
        let mut track_list: Vec<_> = exercises_by_track
            ...
            .collect();
        track_list.sort_unstable_by(|a, b| {
            a["name"]
                .as_str()
                .unwrap_or("")
...
fn progress_bar_helper(
    ...
    let percentage = h.param(0).and_then(|v| v.value().as_u64()).unwrap_or(0) as usize;
...
fn load_track_metadata() -> TrackDataResult<String, TrackMetadata> {
    let home_dir = env::var("HOME").or_else(|_| env::var("USERPROFILE"))?;
    let json_path = Path::new(&home_dir)
        .join("exercism")
        .join("database")
        .join("tracks.json");
    ...
    let metadata: HashMap<_, _> = tracks
        .into_iter()
